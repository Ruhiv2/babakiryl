...
function WinningNumbersTab() {
  ...
  return (
    <div className="max-w-7xl mx-auto p-6">
      <h2 className="text-2xl font-bold text-text-primary mb-6">Manage Winning Numbers</h2>
      <form
      ...
      >
        <div className="grid md:grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-text-primary mb-2">Winning Number (XXX-YYYYYYY)</label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-text-primary mb-2">Position (1-10)</label>
            <input
              type="number"
          ...
          <div>
            <label className="block text-text-primary mb-2">Prize Amount</label>
            <input
              type="number"
          ...
          <table className="w-full bg-white rounded-lg shadow-md">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="p-3 text-text-primary">Winning Number</th>
                <th className="p-3 text-text-primary">Position</th>
                <th className="p-3 text-text-primary">Prize Amount</th>
                <th className="p-3 text-text-primary">Created At</th>
              </tr>
            </thead>
            <tbody>
              {winningNumbers.map((wn) => (
                <tr key={wn.id} className="hover:bg-gray-50">
                  <td className="p-3">{wn.id}</td>
                  <td className="p-3">{wn.draw_date}</td>
                  <td className="p-3">{wn.winning_number}</td>
                  <td className="p-3">{wn.position}</td>
                  <td className="p-3">{wn.prize_amount}</td>
                  <td className="p-3">{new Date(wn.created_at).toISOString().split('T')[0]}</td>
                </tr>
              ))}
            </tbody>
          </table>
